...
public class TogglService {
	...
	TogglProject findOrCreateProject(Long workspaceId, Long clientId, Long issueNumber, String issueTitle) {
		...
		var project = result.getBody().stream().filter(p -> p.getName().startsWith(issueNumber + " -")).findFirst();
	...
	private TogglProject createToggleProject(Long workspaceId, Long clientId, Long issueNumber, String projectName) {
		val uri = UriComponentsBuilder.fromUriString(baseUrl)
			.path("/workspaces/{workspaceId}/projects")
			.build(workspaceId);

		val project = TogglProject.builder()
			.active(true)
			.clientId(clientId)
